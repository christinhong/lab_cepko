...
# Tasks
    # First pass mapping of reads to Galgal5 with STAR via job array (1 job per sample)
...
echo "Finding *_paired_trimmed.fq.gz"
echo

find . *_paired_trimmed.fq.gz -name ".*" -prune -o -print > fq.txt
...
while IFS='' read -r lane || [[ -n "$lane" ]]; do
    find . *$lane*_paired_trimmed.fq.gz  -name ".*" -prune -o -print | paste -sd " "
