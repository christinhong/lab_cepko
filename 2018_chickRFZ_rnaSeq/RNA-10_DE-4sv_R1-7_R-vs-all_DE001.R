...
# Write out annotated data for faster loading later
write.table(
    x = data.frame(fc.ann2, stringsAsFactors = T),
    file = paste0(out.name, "counts_annotated_R1-7.csv"),
    quote = F,
    sep = ",",
    row.names = F
)
...
#### IMPORT annotated and log2(x+1) transform data ####
fc <- read.csv(file = "output/R_counts_annotated_R1-7.csv", header = TRUE, stringsAsFactors = TRUE)
...
pca.fc <- prcomp(fc.log[, coi], center = F, scale. = F)

sink(paste0(out.name, "retinas1-6_pcaSummary.txt"))
summary(pca.fc)
sink()
...
# DESeq2 wants a matrix of raw counts (not log-transformed!) that's gene vs. sample and a table of annotations


## For counts, subset fc for only genes
fc.orig <- fc[6:ncol(fc)]

head(fc.orig[,1:7])


# Transpose
fc.orig.cts <- t(fc.orig)
...
#### Make ID column RFZ vs. other ####
levels(fc.ann$Tissue)

fc.ann$Tissue2 <- fc.ann$Tissue


fc.ann$Tissue <- forcats::fct_collapse(fc.ann$Tissue2,
                                 DNTV = c("D","N", "T", "V"),
                                 RFZ = "RFZ")
...
fc.ann %>% dplyr::filter(Tissue == "RFZ") # Looks good.


## Later on I'll merge technical replicates (Batch 1 and 2), so going to give them the same identifier now
    ### NOTE: In the future, would want to more cleanly distinguish between biological batches (samples collected and prepped under different conditions), libraries, and technical replicates.
...
fc.ann$Retina <- factor(fc.ann$Retina)
...
# NOTE: Later on changed this to only account for Tissue as a variable because I'll be correcting the analysis with svaseq.
...
# DESeq2 recommends collapsing technical replicates = multiple sequencing runs of the same library. In this case means lanes plus batch 1 and batch 2. See non-unique IDs in:
...
ddsC <- collapseReplicates(dds,
    ...
    ## Note: If collapsing batches of technical replicates, could record what's been collapsed by adding "run = dds$Batch"
...
origCts <- rowSums( counts(dds)[ , dds$ID == "DNTV_1" ] )

all( origCts == counts(ddsC)[ ,"DNTV_1" ]) # Should output TRUE
...
res <- results(deAnalysis, name="Tissue_DNTV_vs_RFZ") # Is the same as below
...
pdf(paste0(out.name, "deseq2-svaTest_DNTV-vs-RFZ.pdf"), width=11)
...
#
# NOTE: Later on, this cleaning became unnecessary due to some extra filtering for low count genes plus catching a coding error.  Keeping the code though in case it becomes relevant later.
...
dev.off() # Sometimes need to do this first to get DESeq2 plots to run


pdf(paste0(out.name, "deseq2-dispersionEstimates-4svs_R1-7_DNTV.pdf"), width = 6, height = 6)
...
# DESeq's plotPCA looks exactly the same as without SVs - I mean, *exactly* the same. After spending a couple hours hunting around, I discovered that's how it's supposed to be - standard PCA is for the unadjusted data.  :p  To visualize adjustment, Michael Love (DESeq2 author) suggested limma's removeBatchEffect.
...
rld.cts.sva.t <- t(rld.cts.sva)

write.csv(
    x = data.frame(rld.cts.sva.t, stringsAsFactors = F),
    file = paste0(out.name, "deseq2_allGenes-rlogCounts_R1-7.csv"),
    quote = F,
    row.names = T
)
...
#### POTENTIAL ERROR: Get percent variance explained by each PC ####
...
#### PDF ####
pdf(paste0(out.name, "deseq2-pca-4svs-1000topVar_DNTV-vs-RFZ.pdf"))
...
res.sva <- lfcShrink(deAnalysis.sva, coef="Tissue_DNTV_vs_RFZ", type="normal")
head(res.sva)



#### Adding annotations and HUGO gene symbols ####
# Set variable (if I'm setting a variable, I should probably turn this into a function.  :p  Maybe later.)
df <- res.sva
...
ensembl <- biomaRt::useMart( "ENSEMBL_MART_ENSEMBL",
                             ...
                             ensemblRedirect = FALSE ) # Galgal5.  Specified host and "ensemblRedirect = FALSE" when the local host was down.
...
# Can get gene names for http://bioinformatics.psb.ugent.be/webtools/Venn/
write.csv(
    x = data.frame(res.sva, stringsAsFactors = F),
    file = paste0(out.name, "deseq2-results_DNTV-vs-RFZ_DE.001_R1-7.csv"),
    quote = F,
    row.names = T
)
...
res.sva[res.sva$hgnc_symbol %in% "CYP26C1", ] # Testing search code

res.sva[res.sva$ensembl %in% "ENSGALG00000007706", ] # Ensembl for FGF8. No hgnc_symbol in Gallus gallus 5 genemap!!




#### Import results ####
#res.sva <- read.csv(file = "output/R_deseq2-results_DNTV-vs-RFZ_DE.05_R1-7.csv", header = TRUE, stringsAsFactors = FALSE)
# Importing causes some bugs down the line unless I format it to look exactly like the original - mainly turning first column into rownames.  But don't need it right now.
...
#
# head(ensembl.de)
# length(ensembl.de) # same as total DE genes
#
#
# ensembl.de.uniq <- unique(ensembl.de)
#
# length(ensembl.de.uniq) # 1145 vs. 1209 with Retina 7 included


head(res.sva)
nrow(res.sva) # 897
length(unique(res.sva$ensembl)) # 822.  Not sure why I have some duplicates...maybe from the merge.  Hmm.  Not ideal, but probably good enough.

head(rld2.sva.t)


# Subset for DE genes
rld2.sva.de <- rld2.sva.t[row.names(rld2.sva.t) %in% res.sva$ensembl, ]
...
pdf(paste0(out.name, "deseq2-MAplot-deGenes_R1-7_DNTV.pdf"), width = 10, height = 7)


DESeq2::plotMA(res.sva, main = "DNTV vs. RFZ, \nshrunken log2 fold change,\nred = pAdj < 0.001", ylim = c(-1.5, 1.5))
...
dim(rld2.sva.de.shared) # 28 with Retinas 1-7, 23 with Retinas 1-6 (though that's statistically sketchy), and 62 with Retinas 1-5
...
write.csv(
    ...
    file = paste0(out.name, "deseq2_sharedGenes_DE.05_ensemblIDs_R1-7.csv"),
    quote = F,
    row.names = T
)
...
rld2.sva.de.shared <- read.csv(file = "output/R_deseq2_sharedGenes_DE.05_R1-6_2018-10-11.csv",
                               header = TRUE,
...
pdf(paste0(out.name, "deseq2-heatmap-4svs-deGenes_R1-7_DNTV.pdf"), width = 20, height = 14)
...
#### Make heatmap of Z-scores by gene ####
# Column center by subtracting the mean value for each column (improves heatmap and pca)
rld2.sva.de.mean <- apply(rld2.sva.de, 1, mean)

rld2.sva.de.cent <- rld2.sva.de
rld2.sva.de.cent <- sweep(rld2.sva.de, 1, rld2.sva.de.mean, FUN = "-")



# Scale columns into Z-scores
rld2.sd <- apply(rld2.sva.de.cent, 1, sd)

rld2.sva.de.z <- rld2.sva.de.cent
rld2.sva.de.z <- sweep(rld2.sva.de.cent, 1, rld2.sd, FUN = "/")
...
# Plotting sorted heatmap
pdf(paste0(out.name, "deseq2-heatmap-4svs-deGenes-Zscores_R1-7_DNTV.pdf"), width = 20, height = 14)
...
pheatmap(
    ...
    drop_levels = TRUE,
    fontsize = 20,
    main = "Standard deviation of shared DE genes\n(gene expression centered and scaled to Z-scores)\nDendrogram sorting and quantile color breaks",
    kmeans_k = NA,
    breaks = flowHM_breaks,
    cluster_cols = flowHM_cluster_cols,
    cluster_rows = flowHM_cluster_rows
...
#### Comparing intersect of RFZ DE genes between R1-7 ####
e7.orig <- read.csv(file = "output/R_deseq2_sharedGenes_DE.05_ensemblIDs_R1-7.csv",
                               header = TRUE,
                               stringsAsFactors = FALSE,
                               row.names = 1)

e7 <- cbind(rownames(e7.orig), e7.orig)


e5 <- read.csv(file = "output/R_deseq2_sharedGenes_DE.05_ensemblIDs_R1-5_2018-10-23.csv",
                  header = TRUE,
                  stringsAsFactors = FALSE,
                  row.names = 1)
...
nrow(res.sva) # 821
length(e7$`rownames(e7.orig)`) # 28, good
length(e5$ensembl.de.shared) # 62, good

length(intersect(e7$`rownames(e7.orig)`, e5$ensembl.de.shared)) # 18, what??  Why only 18 shared hits?


e.shared <- intersect(e7$`rownames(e7.orig)`, e5$ensembl.de.shared)


# Writing out shared DE genes from analyzing 1-7 vs. 1-5 and manually adding HUGO gene symbols
write.csv(
    x = data.frame(e.shared, stringsAsFactors = F),
    file = paste0(out.name, "deseq2_sharedGenes_R1-5-vs-R1-7_DE.05_ensemblIDs.csv"),
    quote = F,
    row.names = T
)
