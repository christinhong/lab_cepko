...
pdf(paste0(out.name, "deseq2-dispersionEstimates-4svs_R1-7.pdf"), width = 6, height = 6)
...
write.csv(
    ...
    file = paste0(out.name, "deseq2-results_D-vs-RFZ_DE.05_R1-7.csv"),
    quote = F,
    row.names = T
)
...
write.csv(
    ...
    file = paste0(out.name, "deseq2-results_N-vs-RFZ_DE.05_R1-7.csv"),
    quote = F,
    row.names = T
)
...
write.csv(
    ...
    file = paste0(out.name, "deseq2-results_T-vs-RFZ_DE.05_R1-7.csv"),
    quote = F,
    row.names = T
)
...
write.csv(
    ...
    file = paste0(out.name, "deseq2-results_V-vs-RFZ_DE.05_R1-7.csv"),
    quote = F,
    row.names = T
)
...
#### Import results ####
res.sva.DR <- read.csv(file = "output/R_deseq2-results_D-vs-RFZ_DE.05_R1-7.csv", header = TRUE, stringsAsFactors = FALSE)


res.sva.NR <- read.csv(file = "output/R_deseq2-results_N-vs-RFZ_DE.05_R1-7.csv", header = TRUE, stringsAsFactors = FALSE)


res.sva.TR <- read.csv(file = "output/R_deseq2-results_T-vs-RFZ_DE.05_R1-7.csv", header = TRUE, stringsAsFactors = FALSE)


res.sva.VR <- read.csv(file = "output/R_deseq2-results_V-vs-RFZ_DE.05_R1-7.csv", header = TRUE, stringsAsFactors = FALSE)
...
pdf(paste0(out.name, "deseq2-MAplot-deGenes_R1-7.pdf"), width = 10, height = 7)
...
# Plotting sorted heatmap
pdf(paste0(out.name, "deseq2-heatmap-4svs-SharedDEGenes_Retinas1-6_", Sys.Date(), ".pdf"), width = 20, height = 14)
...
# Column center by subtracting the mean value for each column (improves heatmap and pca)
rld2.sva.de.shared.mean <- apply(rld2.sva.de.shared, 1, mean)

rld2.sva.de.shared.cent <- rld2.sva.de.shared
rld2.sva.de.shared.cent <- sweep(rld2.sva.de.shared, 1, rld2.sva.de.shared.mean, FUN = "-")



# Checking for GAS2L3
rld2.sva.de.shared.cent[rownames(rld2.sva.de.shared.cent) %in% "GAS2L3", ]




# Scale columns into Z-scores
rld2.shared.sd <- apply(rld2.sva.de.shared.cent, 1, sd)

rld2.sva.de.shared.z <- rld2.sva.de.shared.cent
rld2.sva.de.shared.z <- sweep(rld2.sva.de.shared.cent, 1, rld2.shared.sd, FUN = "/")
...
# Plotting sorted heatmap
pdf(paste0(out.name, "deseq2-heatmap-4svs-SharedDEGenes-geneZscores_Retinas1-6_", Sys.Date(), ".pdf"), width = 20, height = 14)
...
#### Comparing intersect of RFZ DE genes between R1-7 and R1-5 ####
