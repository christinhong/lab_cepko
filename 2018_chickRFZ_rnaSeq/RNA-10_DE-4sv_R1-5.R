...
#### Remove Retina 6 and 7 from analysis ####
head(fc.ann2)

fc.sub <- fc.ann2 %>% dplyr::filter(Retina != 6 & Retina != 7)


# Check
fc.sub[,1:10] %>% dplyr::filter(Retina == 5 | Retina == 6) # Looks good.
...
write.table(
    x = data.frame(fc.sub, stringsAsFactors = T),
    file = paste0(out.name, "counts_annotated_R1-5.csv"),
    quote = F,
    sep = ",",
    row.names = F
)
...
fc <- read.csv(file = paste0(out.name, "counts_annotated_R1-5.csv"), header = TRUE, stringsAsFactors = TRUE)
...
sink(paste0(out.name, "retinas1-5_pcaSummary.txt"))
summary(pca.fc)
sink()
...
#### Import annotated data ####
fc <- read.csv(file = "output/R_counts_annotated_R1-5.csv", header = TRUE, stringsAsFactors = TRUE)
...
#### Collapse lanes and technical replicates ####
...
ddsC <- collapseReplicates(dds,
    ...
    # If I were collapsing on only one variable, could track that with "run = dds$Batch".
...
# DESeq's plotPCA
DESeq2::plotPCA(rld, intgroup = "Tissue", ntop = 1000, returnData = FALSE)
...
gg <- DESeq2::plotPCA( rld, intgroup = c( "Tissue"), returnData = TRUE, ntop = 1000)
...
#     file = paste0(out.name, "deseq2_allGenes-rlogCounts_R1-5.csv"),
#     quote = F,
#     row.names = T
# )
#
...
rld2.cts <- rld2[, 3:ncol(rld2)]

# Transpose
rld2.cts.t <- t(rld2.cts)

head(rld2.cts.t[,1:10])
class(rld2.cts.t) # Checking for numeric matrix


# Estimate variance for each row
var_genes <- apply(rld2.cts.t, 1, var)
...
rld2.var <- rld2.cts.t[select_var,]
...
rld2.var.t <- t(rld2.var)
head(rld2.var.t)


# Annotate count data
rld2.var <- cbind(rld.ann3, rld2.var.t)
head(rld2.var[,1:6])



#### Heatmap ####
sampleDists <- dist(rld2.var.t)

sampleDistMatrix <- as.matrix(sampleDists)

rownames(sampleDistMatrix) <- paste(rld2$Retina, rld2$Tissue, sep="-")
...
pcVar <- round((((pca.rld$sdev)^2) * 100) / sum(pca.rld$sdev^2), digits = 2)

cumsum((pca.rld$sdev)^2) / sum(pca.rld$sdev^2)

#### PDF ####
pdf(paste0(out.name, "deseq2-pca-4svs_R1-5_", Sys.Date(), ".pdf"))
...
pca.rld.var <- prcomp(rld2.var.t, center = F, scale. = F)
...
plot(pca.rld.var, type = "lines", main = "PCA of RNA-seq by prcomp")
title(xlab = "Principle Components")

head(pca.rld.sva.var$x)


scores.var <- data.frame(rld2.var.t, pca.rld.var$x[,1:9], rld2[,1:2])


# Get percent variance explained by each PC
head(pca.rld.var$sdev)

pcVar <- round((((pca.rld.var$sdev)^2) * 100) / sum(pca.rld.var$sdev^2), digits = 2)

cumsum((pca.rld.var$sdev)^2) / sum(pca.rld.var$sdev^2)


#### PDF ####
pdf(paste0(out.name, "deseq2-pca-4svs-1000topVar_R1-5_", Sys.Date(), ".pdf"))
...
# SVA
ggplot(data = scores.var, aes(x = PC1, y = PC2)) +
    geom_point(aes(colour = factor(Retina)), size = 4) +
    theme(legend.position="right") +
    theme_light() +
    ggtitle("(PCA determined by prcomp, \nPC1 scaled by 0.2 for easier viewing)") +
...
res.sva.DR <- lfcShrink(deAnalysis, coef="Tissue_D_vs_RFZ", type="normal")
res.sva.NR <- lfcShrink(deAnalysis, coef="Tissue_N_vs_RFZ", type="normal")
res.sva.TR <- lfcShrink(deAnalysis, coef="Tissue_T_vs_RFZ", type="normal")
res.sva.VR <- lfcShrink(deAnalysis, coef="Tissue_V_vs_RFZ", type="normal")
...
res.sva.DR.orig <- results(deAnalysis, coef="Tissue_D_vs_RFZ")

pdf(paste0(out.name, "deseq2-MA-4svs-DvsRFZ_R1-5_", Sys.Date(), ".pdf"), width = 14, height = 6)
...
write.csv(
    ...
    file = paste0(out.name, "deseq2-results_D-vs-RFZ_DE.05_R1-5.csv"),
    quote = F,
    row.names = T
)
...
write.csv(
    ...
    file = paste0(out.name, "deseq2-results_N-vs-RFZ_DE.05_R1-5.csv"),
    quote = F,
    row.names = T
)
...
write.csv(
    ...
    file = paste0(out.name, "deseq2-results_T-vs-RFZ_DE.05_R1-5.csv"),
    quote = F,
    row.names = T
)
...
write.csv(
    x = data.frame(res.sva.VR, stringsAsFactors = F),
    file = paste0(out.name, "deseq2-results_V-vs-RFZ_DE.05_R1-5.csv"),
    quote = F,
    row.names = T
)
...
res.sva.VR[res.sva.VR$hgnc_symbol %in% "CYP26C1", ] # Testing search code
...
#### Import results ####
res.sva.DR <- read.csv(file = "output/R_deseq2-results_D-vs-RFZ_DE.0.csv", header = TRUE, stringsAsFactors = FALSE)
...
length(ensembl.de.uniq) # 1145 vs. 1209 with Retina 7 included.  2747 for R1-5.
...
pdf(paste0(out.name, "deseq2-MAplot-deGenes_R1-5.pdf"), width = 10, height = 7)

par(mfrow = c(2, 2))

DESeq2::plotMA(res.sva.DR, main = "D vs. RFZ, \nshrunken log2 fold change,\nred = pAdj < 0.05", ylim = c(-1.5, 1.5))

DESeq2::plotMA(res.sva.NR, main = "N vs. RFZ, \nshrunken log2 fold change,\nred = pAdj < 0.05", ylim = c(-1.5, 1.5))

DESeq2::plotMA(res.sva.TR, main = "T vs. RFZ, \nshrunken log2 fold change,\nred = pAdj < 0.05", ylim = c(-1.5, 1.5))

DESeq2::plotMA(res.sva.VR, main = "V vs. RFZ, \nshrunken log2 fold change,\nred = pAdj < 0.05", ylim = c(-1.5, 1.5))

dev.off()



#### Get Z-scores for standard deviations ####
...
# Write out list of shared Ensembl IDs
write.csv(
    x = data.frame(ensembl.de.shared, stringsAsFactors = F),
    file = paste0(out.name, "deseq2_sharedGenes_DE.05_ensemblIDs_R1-5_", Sys.Date(), ".csv"),
    quote = F,
    row.names = T
)
...
# Subset for DE genes
rld2.t <- t(rld2[, 3:ncol(rld2)])
head(rld2.t)
nrow(rld2.t)


rld2.de.shared <- rld2.t[row.names(rld2.t) %in% ensembl.de.shared, ]

nrow(rld2.de.shared) # 23 instead of 28 with Retina 7.  62 with Retina 1-5??  Is that right...?

head(rld2.de.shared[,1:6])
...
write.csv(
    ...
    file = paste0(out.name, "deseq2_sharedGenes_DE.05_hgncSymbolAdded_R1-5", Sys.Date(), ".csv"),
    quote = F,
    row.names = T
)
...
rld2.de.shared <- as.matrix(rld2.de.shared)

class(rld2.de.shared)



#### Make non-standardized (shrunken log2(x+1) counts) heatmap ####
df <- rld2.de.shared
...
# Plotting sorted heatmap
pdf(paste0(out.name, "deseq2-heatmap-4svs-SharedDEGenes_Retinas1-5_", Sys.Date(), ".pdf"), width = 20, height = 14)
...
#### Standardize data by gene ####
# Column center by subtracting the mean value for each column (improves heatmap and pca)
rld2.de.shared.mean <- apply(rld2.de.shared, 1, mean)

rld2.de.shared.cent <- rld2.de.shared
rld2.de.shared.cent <- sweep(rld2.de.shared, 1, rld2.de.shared.mean, FUN = "-")



# Scale columns by standard deviation (into Z-scores)
rld2.shared.sd <- apply(rld2.de.shared.cent, 1, sd)

rld2.de.shared.z <- rld2.de.shared.cent
rld2.de.shared.z <- sweep(rld2.de.shared.cent, 1, rld2.shared.sd, FUN = "/")
...
# Plotting sorted heatmap
pdf(paste0(out.name, "deseq2-heatmap-4svs-SharedDEGenes-geneZscores_Retinas1-5_", Sys.Date(), ".pdf"), width = 20, height = 14)
