...
After the 2nd STAR pass, the percentage of reads that were "mapped to too many loci" in the first pass seem to split across "mapping to too many loci" and "reads unmapped: other". But the percentage of uniquely mapped reads is stable, so that's probably fine.
