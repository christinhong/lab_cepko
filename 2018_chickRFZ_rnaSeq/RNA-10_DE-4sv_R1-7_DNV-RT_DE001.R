...
# Output
out.name <- paste0("output/DNV-RT/R_")
...
fc.ann %>% dplyr::filter(Tissue == "DNV")
fc.ann %>% dplyr::filter(Tissue == "RT") # Looks good.
...
origCts <- rowSums( counts(dds)[ , dds$ID == "DNV_1" ] )

all( origCts == counts(ddsC)[ ,"DNV_1" ]) # Should output TRUE
...
ddsC$Tissue <- relevel( ddsC$Tissue, "RT" )
...
res <- results(deAnalysis, name="Tissue_DNV_vs_RT") # Is the same as below
...
pdf(paste0(out.name, "deseq2-svaTest_DNV-vs-RT.pdf"), width=11)
...
pdf(paste0(out.name, "deseq2-dispersionEstimates-4svs_R1-7_DNV-RT.pdf"), width = 6, height = 6)
...
pdf(paste0(out.name, "deseq2-heatmap-4svs-1000topVar_R1-7_DNV-RT.pdf"))
...
pcVar <- round((((pca.rld.sva.var$sdev)^2) / sum(pca.rld.sva.var$sdev^2)) * 100, digits = 2)
...
#### Identifying and extracting DE genes ####
...
res.sva <- lfcShrink(deAnalysis.sva, coef="Tissue_DNV_vs_RT", type="normal")
head(res.sva)
...
write.csv(
    ...
    file = paste0(out.name, "deseq2-results_DNV-vs-RT_DE.001_R1-7.csv"),
    quote = F,
    row.names = T
)
...
head(res.sva)
nrow(res.sva) # 182
length(unique(res.sva$ensembl)) # 182
...
pdf(paste0(out.name, "deseq2-MAplot-deGenes_R1-7_DNV-RT.pdf"), width = 10, height = 7)


DESeq2::plotMA(res.sva, main = "DNV vs. RT, \nshrunken log2 fold change,\nred = pAdj < 0.001", ylim = c(-1.5, 1.5))
...
pdf(paste0(out.name, "deseq2-heatmap-4svs-deGenes_R1-7_DNV-RT.pdf"), width = 20, height = 14)
...
# Plotting sorted heatmap
pdf(paste0(out.name, "deseq2-heatmap-4svs-deGenes-Zscores_R1-7_DNV-RT.pdf"), width = 20, height = 14)
...
pheatmap(
    ...
    drop_levels = TRUE,
    fontsize = 20,
    main = "Standard deviation of DE genes between DNV and RT (182)\n(gene expression centered and scaled to Z-scores)\nDendrogram sorting and quantile color breaks",
    kmeans_k = NA,
    breaks = flowHM_breaks,
    cluster_cols = flowHM_cluster_cols,
    cluster_rows = flowHM_cluster_rows
