...
#### Remove Retina 7 from analysis ####
head(fc.ann2)

fc.sub <- fc.ann2 %>% dplyr::filter(Retina != 7)


# Check
fc.sub[,1:10] %>% dplyr::filter(Retina == 6 | Retina == 7) # Looks good.
...
write.table(
    ...
    file = paste0(out.name, "counts_annotated_R1-6.csv"),
    quote = F,
    sep = ",",
    row.names = F
)
...
fc <- read.csv(file = "output/R_counts_annotated_R1-6.csv", header = TRUE, stringsAsFactors = TRUE)
...
write.csv(
    ...
    file = paste0(out.name, "deseq2_allGenes-rlogCounts_", Sys.Date(), ".csv"),
    quote = F,
    row.names = T
)
...
dim(rld2.sva.de.shared) # 23 instead of 28 with Retina 7
...
write.csv(
    ...
    file = paste0(out.name, "deseq2_sharedGenes_DE.05_R1-6_", Sys.Date(), ".csv"),
    quote = F,
    row.names = T
)
